...
def unanswered():
    ...
    db = get_db()
    unanswered_cur = db.execute('''select questions.id, question_text, users.name as name
                                   from questions 
                                   join users on users.id == questions.asked_by_id
                                   where expert_id = ? and answer_text is null''', [user['id']])
    unanswered_result = unanswered_cur.fetchall()

    return render_template('unanswered.html', user=user, unanswered = unanswered_result)
